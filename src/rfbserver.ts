...
export class RFBServer {
    ...
    private async handleConnection(socket: net.Socket) {
        ...
        try {
            ...
            if (e instanceof SocketBufferEndedError) {
    ...
    private async handleTlsConnection(socket: tls.TLSSocket) {
        ...
        try {
            ...
            // Read credentials
            username = (await buffer.readNBytesOffset(usernameLength)).toString('utf8');
            password = (await buffer.readNBytesOffset(passwordLength)).toString('utf8');
